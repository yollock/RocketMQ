        ...
        if (consumerGroupInfo != null && !consumerGroupInfo.getAllChannel().isEmpty()) {
            ...
            for (Channel channel : channelInfoTable.keySet()) {
                ...
                if (version >= MQVersion.Version.V3_0_7_SNAPSHOT.ordinal()) {
                    ...
                    log.warn("[reset-offset] the client does not support this feature. version={}", RemotingHelper.parseChannelRemoteAddr(channel), MQVersion.getVersionDesc(version));
                    return response;
                }
            }
        }
    ...
    public RemotingCommand getConsumeStatus(String topic, String group, String originClientId) {
        ...
        for (Channel channel : channelInfoTable.keySet()) {
            ...
            if (version < MQVersion.Version.V3_0_7_SNAPSHOT.ordinal()) {
                ...
                log.warn("[get-consumer-status] the client does not support this feature. version={}", RemotingHelper.parseChannelRemoteAddr(channel), MQVersion.getVersionDesc(version));
                return result;
            } else if (UtilAll.isBlank(originClientId) || originClientId.equals(clientId)) {
                // 不指定 originClientId 则对所有的 client 进行处理；若指定 originClientId 则只对当前
                // originClientId 进行处理
                try {
                    RemotingCommand response = this.brokerController.getRemotingServer().invokeSync(channel, request, 5000);
